...
上穿1原:=CROSS(PCTB,1);
STICKLINE(显示许可 AND 轨上,0.04,0.14,1,0),COLORCYAN;
STICKLINE(显示许可 AND 手工上穿,0.20,0.34,2,0),COLORWHITE;
STICKLINE(显示许可 AND 上穿1原,0.40,0.56,3,0),COLORYELLOW;
DRAWTEXT(显示许可 AND 手工上穿,0.36,'手穿'),COLORWHITE;
DRAWTEXT(显示许可 AND 上穿1原,0.58,'CROSS'),COLORYELLOW;
...
STICKLINE(显示许可 AND 浅绕回上穿,0.62,0.74,2,0),COLORGRAY;
STICKLINE(显示许可 AND 有效上穿1,0.80,0.96,3,0),COLORRED;
DRAWTEXT(显示许可 AND 浅绕回上穿,0.76,'浅'),COLORGRAY;
DRAWTEXT(显示许可 AND 有效上穿1,0.98,'效穿'),COLORRED;
...
背景昨缩:=REF(真正收缩,1);
背景今缩:=真正收缩;
突破背景简:=背景近挤 OR 迟发简化背景 OR 背景昨缩 OR 背景今缩;
STICKLINE(显示许可 AND 突破背景简,1.04,1.18,2,0),COLORGREEN;
DRAWTEXT(显示许可 AND 突破背景简,1.20,'背'),COLORGREEN;

{读法：青=轨上；白=手工上穿；黄=CROSS；灰=浅绕；红=有效上穿；绿=突破背景。}`;

fs.writeFileSync('validation-indicator-A3a-crosscheck-draft.txt', formula, 'utf8');
console.log('generated validation-indicator-A3a-crosscheck-draft.txt');
