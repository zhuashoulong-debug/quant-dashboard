...
迟发宽速许可:=宽速率<=微扩阈;
迟发宽未扩:=迟发绝宽许可 AND 迟发近宽未破 AND 迟发均宽许可 AND NOT(启动前持宽) AND NOT(REF(宽近高,1)) AND 迟发宽速许可;
...
STICKLINE(显示许可 AND 背景近挤,0.08,0.14,1,0),COLORWHITE;
STICKLINE(显示许可 AND 迟发挤压背景,0.16,0.22,1,0),COLORMAGENTA;
STICKLINE(显示许可 AND 背景昨缩,0.24,0.30,1,0),COLORCYAN;
STICKLINE(显示许可 AND 背景今缩,0.32,0.38,1,0),COLORGREEN;
STICKLINE(显示许可 AND 突破背景,0.40,0.46,1,0),COLORRED;
DRAWTEXT(显示许可 AND 迟发挤压背景,0.24,'迟'),COLORMAGENTA;
DRAWTEXT(显示许可 AND 背景今缩,0.40,'缩'),COLORGREEN;
...
STICKLINE(显示许可 AND 上穿1原,0.54,0.60,1,0),COLORWHITE;
STICKLINE(显示许可 AND 浅绕回上穿,0.62,0.68,1,0),COLORGRAY;
STICKLINE(显示许可 AND 有效上穿1,0.70,0.76,2,0),COLORYELLOW;
DRAWTEXT(显示许可 AND 有效上穿1,0.78,'穿'),COLORYELLOW;
DRAWTEXT(显示许可 AND 浅绕回上穿,0.70,'浅'),COLORGRAY;

温上基:=有效上穿1 AND 突破背景 AND PCTB<1.1 AND REF(PCTB,1)<1;
温上:=温上基 AND (温额 OR 构上) AND NOT(长上);
STICKLINE(显示许可 AND 温上基,0.82,0.88,1,0),COLORWHITE;
STICKLINE(显示许可 AND 温上,0.90,0.98,2,0),COLORYELLOW;
DRAWTEXT(显示许可 AND 温上,1.00,'温'),COLORYELLOW;

暴上原:=有效上穿1 AND 突破背景 AND REF(PCTB,1)<1 AND PCTB>=1.1 AND PCTB<1+深限;
暴上:=暴上原 AND (放额 OR 构上) AND 阳强 AND NOT(长上);
暴上险:=暴上原 AND 长上;
STICKLINE(显示许可 AND 暴上原,1.02,1.08,1,0),COLORWHITE;
STICKLINE(显示许可 AND 暴上,1.10,1.18,2,0),COLORRED;
STICKLINE(显示许可 AND 暴上险,1.10,1.18,2,0),COLORMAGENTA;
DRAWTEXT(显示许可 AND 暴上,1.20,'暴'),COLORRED;
DRAWTEXT(显示许可 AND 暴上险,1.20,'暴险'),COLORMAGENTA;

极上原:=突破背景 AND REF(PCTB,1)<1 AND PCTB>=1+深限;
极上:=极上原 AND (放额 OR 构上) AND 阳强 AND NOT(长上);
极上险:=极上原 AND (长上 OR 巨额);
STICKLINE(显示许可 AND 极上原,1.22,1.26,1,0),COLORWHITE;
STICKLINE(显示许可 AND 极上,1.24,1.30,2,0),COLORRED;
STICKLINE(显示许可 AND 极上险,1.24,1.30,2,0),COLORMAGENTA;
DRAWTEXT(显示许可 AND 极上,1.28,'极'),COLORRED;
DRAWTEXT(显示许可 AND 极上险,1.28,'极险'),COLORMAGENTA;

{阅读：低位=背景来源；中位=上穿与浅绕；高位=温/暴/极及其原始层。}`;

fs.writeFileSync('validation-indicator-A3a-upbreak-draft.txt', formula, 'utf8');
console.log('generated validation-indicator-A3a-upbreak-draft.txt');
