...
STICKLINE(显示许可 AND 阳强,0.04,0.12,1,0),COLORRED;
STICKLINE(显示许可 AND 长上,0.16,0.24,1,0),COLORMAGENTA;
DRAWTEXT(显示许可 AND 阳强 AND NOT(REF(阳强,1)),0.14,'阳'),COLORRED;
DRAWTEXT(显示许可 AND 长上 AND NOT(REF(长上,1)),0.26,'长'),COLORMAGENTA;
...
STICKLINE(显示许可 AND 温额,0.32,0.40,1,0),COLORYELLOW;
STICKLINE(显示许可 AND 放额,0.44,0.54,1,0),COLORRED;
STICKLINE(显示许可 AND 巨额,0.58,0.68,1,0),COLORMAGENTA;
DRAWTEXT(显示许可 AND 放额 AND NOT(REF(放额,1)),0.56,'放'),COLORRED;
DRAWTEXT(显示许可 AND 巨额 AND NOT(REF(巨额,1)),0.70,'巨'),COLORMAGENTA;

实体高:=MAX(C,O);
构上:=C>REF(HHV(实体高,BRN),1);
STICKLINE(显示许可 AND 构上,0.74,0.84,1,0),COLORWHITE;
DRAWTEXT(显示许可 AND 构上,0.86,'构'),COLORWHITE;
...
迟发宽速许可:=宽速率<=微扩阈;
迟发简化未扩:=迟发近宽未破 AND 迟发均宽许可 AND 迟发宽速许可;
迟发简化未扩:=迟发简化未扩 AND NOT(启动前持宽) AND NOT(REF(宽近高,1));
迟发挤压背景:=迟发曾挤 AND 迟发简化未扩;
...
突破背景:=REF(近挤,1) OR 迟发挤压背景 OR REF(真正收缩,1) OR 真正收缩;
...
源原:=温上基 OR 暴上原 OR 极上原;
STICKLINE(显示许可 AND 源原,0.92,1.00,1,0),COLORWHITE;
DRAWTEXT(显示许可 AND 源原,1.02,'原'),COLORWHITE;
...
STICKLINE(显示许可 AND 温上,1.06,1.20,2,0),COLORYELLOW;
STICKLINE(显示许可 AND 暴上,1.06,1.20,2,0),COLORRED;
STICKLINE(显示许可 AND 极上,1.06,1.20,2,0),COLORMAGENTA;
STICKLINE(显示许可 AND (暴上险 OR 极上险),1.22,1.30,2,0),COLORMAGENTA;
DRAWTEXT(显示许可 AND 温上,1.22,'温'),COLORYELLOW;
DRAWTEXT(显示许可 AND 暴上,1.22,'暴'),COLORRED;
DRAWTEXT(显示许可 AND 极上,1.22,'极'),COLORMAGENTA;
DRAWTEXT(显示许可 AND 暴上险,1.30,'暴险'),COLORMAGENTA;
DRAWTEXT(显示许可 AND 极上险,1.30,'极险'),COLORMAGENTA;

{读法：低区=阳强/长上/量能/构上；白原=温暴极原始层；高区=温暴极或风险源。}`;

fs.writeFileSync('validation-indicator-A3a3-upbreak-quality-mini-draft.txt', formula, 'utf8');
console.log('generated validation-indicator-A3a3-upbreak-quality-mini-draft.txt');
