...
背景关键:=突破背景 AND (PCTB>0.80 OR REF(PCTB,1)>0.80);
STICKLINE(显示许可 AND 背景关键,0.08,0.18,1,0),COLORGREEN;
DRAWTEXT(显示许可 AND 背景关键 AND NOT(REF(背景关键,1)),0.20,'背'),COLORGREEN;
...
STICKLINE(显示许可 AND 上穿1原,0.36,0.48,1,0),COLORWHITE;
STICKLINE(显示许可 AND 有效上穿1,0.52,0.68,2,0),COLORYELLOW;
DRAWTEXT(显示许可 AND 有效上穿1,0.70,'穿'),COLORYELLOW;
DRAWTEXT(显示许可 AND 浅绕回上穿 AND 上穿1原,0.50,'浅'),COLORGRAY;
...
STICKLINE(显示许可 AND 上穿源原,0.82,0.92,1,0),COLORWHITE;
STICKLINE(显示许可 AND 温上,0.98,1.12,2,0),COLORYELLOW;
STICKLINE(显示许可 AND 暴上,0.98,1.12,2,0),COLORRED;
STICKLINE(显示许可 AND 极上,0.98,1.12,2,0),COLORMAGENTA;
STICKLINE(显示许可 AND (暴上险 OR 极上险),1.16,1.28,2,0),COLORMAGENTA;
DRAWTEXT(显示许可 AND 温上,1.14,'温'),COLORYELLOW;
DRAWTEXT(显示许可 AND 暴上,1.14,'暴'),COLORRED;
DRAWTEXT(显示许可 AND 极上,1.14,'极'),COLORMAGENTA;
DRAWTEXT(显示许可 AND 暴上险,1.28,'暴险'),COLORMAGENTA;
DRAWTEXT(显示许可 AND 极上险,1.28,'极险'),COLORMAGENTA;

{读法：绿=突破背景；白=原始上穿/源原；黄=有效上穿；黄/红/紫文字=温暴极；紫高位=风险源。}`;

fs.writeFileSync('validation-indicator-A3a-readable-draft.txt', formula, 'utf8');
console.log('generated validation-indicator-A3a-readable-draft.txt');
