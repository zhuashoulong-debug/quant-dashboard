...
STICKLINE(显示许可 AND 挤压,0.10,0.16,1,0),COLORMAGENTA;
STICKLINE(显示许可 AND 效挤,0.18,0.26,2,0),COLORCYAN;
STICKLINE(显示许可 AND 分散挤原,0.28,0.36,2,0),COLORCYAN;
...
STICKLINE(显示许可 AND 启动前持宽,1.04,1.10,2,0),COLORGRAY;
STICKLINE(显示许可 AND 宽近高,1.12,1.18,2,0),COLORGRAY;
STICKLINE(显示许可 AND 首扩宽度许可,1.20,1.26,1,0),COLORRED;
DRAWTEXT(显示许可 AND 启动前持宽,1.12,'持'),COLORGRAY;
DRAWTEXT(显示许可 AND 宽近高,1.20,'近高'),COLORGRAY;

{三、有效挤压与迟发挤压，算完马上画}
挤压宽许可:=带宽/MAX(前有效宽均,0.0001)<1.15 AND NOT(启动前持宽) AND NOT(REF(宽近高,1));
有效挤压:=(REF(效挤,1) OR 分散挤原) AND 挤压宽许可;
STICKLINE(显示许可 AND 有效挤压,0.38,0.48,3,0),COLORCYAN;
DRAWTEXT(显示许可 AND 有效挤压,0.50,'有挤'),COLORCYAN;
...
迟发宽速许可:=宽速率<=微扩阈;
迟发宽未扩:=迟发绝宽许可 AND 迟发近宽未破 AND 迟发均宽许可 AND 迟发宽速许可 AND NOT(启动前持宽) AND NOT(REF(宽近高,1));
迟发挤压背景:=迟发曾挤 AND 迟发宽未扩;
STICKLINE(显示许可 AND 迟发挤压背景,0.94,1.02,2,0),COLORFF9900;
DRAWTEXT(显示许可 AND 迟发挤压背景,1.04,'迟'),COLORFF9900;
...
STICKLINE(显示许可 AND 收缩幅度,0.54,0.60,1,0),COLORGREEN;
STICKLINE(显示许可 AND 压缩低位,0.62,0.68,1,0),COLORGREEN;
STICKLINE(显示许可 AND 有效缩窄,0.70,0.76,1,0),COLORGREEN;
STICKLINE(显示许可 AND 真正收缩,0.78,0.88,3,0),COLORGREEN;
DRAWTEXT(显示许可 AND 真正收缩,0.90,'真缩'),COLORGREEN;

{A2-front说明：低区=挤压，中区=收缩，高区=迟发/宽度。按算完马上画结构编写。}
`;

fs.writeFileSync('validation-indicator-A2-front-draft.txt', formula, 'utf8');
console.log('generated validation-indicator-A2-front-draft.txt');
