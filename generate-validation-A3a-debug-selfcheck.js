...
背景昨缩:=REF(真正收缩,1);
背景今缩:=真正收缩;
突破背景:=背景近挤 OR 迟发简化背景 OR 背景昨缩 OR 背景今缩;
STICKLINE(显示许可,0.04,0.08,1,0),COLORBLUE;
STICKLINE(显示许可 AND 突破背景,0.10,0.24,3,0),COLORGREEN;
DRAWTEXT(显示许可,0.26,'后1'),COLORBLUE;
...
STICKLINE(显示许可,0.40,0.44,1,0),COLORBLUE;
STICKLINE(显示许可 AND 上穿1原,0.48,0.60,2,0),COLORWHITE;
STICKLINE(显示许可 AND 有效上穿1,0.64,0.78,3,0),COLORYELLOW;
DRAWTEXT(显示许可,0.80,'后2'),COLORBLUE;
DRAWTEXT(显示许可 AND 上穿1原,0.62,'原穿'),COLORWHITE;
...
STICKLINE(显示许可,0.94,0.98,1,0),COLORBLUE;
STICKLINE(显示许可 AND 上穿源原,1.02,1.12,2,0),COLORWHITE;
STICKLINE(显示许可 AND 上穿源成,1.16,1.28,3,0),COLORRED;
DRAWTEXT(显示许可,1.00,'后3'),COLORBLUE;
DRAWTEXT(显示许可 AND 上穿源原,1.14,'源原'),COLORWHITE;
DRAWTEXT(显示许可 AND 上穿源成,1.28,'源成'),COLORRED;

{读法：蓝色后1/后2/后3是后段绘图自检；蓝色不显示则说明后段绘图失效；蓝色显示但其他色不显示才说明条件未触发。}`;

fs.writeFileSync('validation-indicator-A3a-debug-selfcheck-draft.txt', formula, 'utf8');
console.log('generated validation-indicator-A3a-debug-selfcheck-draft.txt');
